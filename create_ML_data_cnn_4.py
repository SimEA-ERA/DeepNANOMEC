...
for i in range(1,11): 
  PATH = "../Raw_data_4/Results"+str(i)+"/Results"   
...
#    norm_stress(file)
    ...
    for j in range(file.shape[0]):
        ...
        if len(inp_sample_list)==256 and j<29440:
            ...
            inp_array = np.zeros([256,13],dtype=np.float64)
            tar_array = np.zeros([256,3],dtype=np.float64)                
        elif len(inp_sample_list)==29 and j>=29440:
...
test_input = input_file[900:]
test_target = target_file[900:]


#c = [*range(1350)]
#np.random.shuffle(c)
train_input = input_file[:800]
train_target = target_file[:800]


val_input = input_file[800:900]
val_target = target_file[800:900]
...
with open('train_target_4.pkl','wb') as f:
      pickle.dump(train_target, f) 

with open('train_input_4.pkl','wb') as f:
      pickle.dump(train_input, f) 


with open('test_target_4.pkl','wb') as f:
      pickle.dump(test_target, f) 

with open('test_input_4.pkl','wb') as f:
      pickle.dump(test_input, f) 


with open('val_target_4.pkl','wb') as f:
      pickle.dump(val_target, f) 

with open('val_input_4.pkl','wb') as f:
