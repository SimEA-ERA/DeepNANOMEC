...
for i in range(1,16): 
  PATH = "../Raw_data_2/Results"+str(i)+"/Results"   
...
#    norm_stress(file)
    ...
    for j in range(file.shape[0]):
        ...
        if len(inp_sample_list)==256 and j<7936:
            ...
            inp_array = np.zeros([256,13],dtype=np.float64)
            tar_array = np.zeros([256,3],dtype=np.float64)                
        elif len(inp_sample_list)==83 and j>=7936:
          
            inp_sample_list = np.array(inp_sample_list,dtype=np.float64)
            tar_sample_list = np.array(tar_sample_list,dtype=np.float64)
            
            inp_array[87:87+83]=inp_sample_list[:]
            tar_array[87:87+83]=tar_sample_list[:]
...
with open('train_target_2.pkl','wb') as f:
...
with open('test_target_2.pkl','wb') as f:
      pickle.dump(test_target, f) 

with open('test_input_2.pkl','wb') as f:
      pickle.dump(test_input, f) 


with open('val_target_2.pkl','wb') as f:
      pickle.dump(val_target, f) 

with open('val_input_2.pkl','wb') as f:
